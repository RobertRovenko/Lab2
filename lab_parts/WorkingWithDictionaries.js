...
export default function WorkingWithDictionaries() {
  const [list, setlist] = useState({ onions: 5, garlic: 3, bread: 1 });
    ...
    <SafeAreaView style={styles.content}>
      <ScrollView>
        ...
        <View style={{ ...styles.section, backgroundColor: "#ffeb7a" }}>
          ...
          <Text style={{ ...styles.bodyText }}>Onions: {list.onions}</Text>
          <Text style={{ ...styles.bodyText }}>Garlic: {list.garlic}</Text>
          <Text style={{ ...styles.bodyText }}>Bread: {list.bread}</Text>
          <Button
            title="Update bread count to 2"
            onPress={() => {
              setlist({ ...list, bread: 2 });
            }}
          />
        </View>
